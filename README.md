...
- **프로모션 증정품과 엮이지 않는 상품**에 한하여 적용된다.
    - 프로모션 상품이어도 프로모션이 적용되지 않아 증정품이 제공되지 않았다면 해당 갯수에 한하여 멤버십이 적용된다!
    - ex1) 2+1 사이다를 7개 구매했을 시 3개 단위 프로모션으로 6개까지는 증정품과 엮이므로 안되지만, 나머지 1개는 프로모션과 관계없으므로 멤버십이 적용된다.
    - ex2) 1+1 주스 재고가 9개이며 사용자가 9개를 모두 구매하고 증정품을 제공받지 않고 결제를 한다면 8개는 적용되고 나머지 1개는 프로모션 미적용으로 멤버십이 적용된다.
- 총액의 **10%**가 할인된다.
...
| 상품  | 프로모션 | 구매 갯수         | 적용 여부            |
  |-----|------|---------------|------------------|
| 주스  | 1+1  | 4개            | ❌ 제외             |
| 주스  | 1+1  | 9개(재고9개)      | ⭕ 1개에 대해 멤버십 적용  |
| 콜라  | 2+1  | 5개(추가 증정 선택x) | ⭕ 2개에 대해 멤버십 적용  |
| 사이다 | 2+1  | 7개            | ⭕ 1개에 대해  멤버십 적용 |
| 삼다수 | 없음   | 3개            | ⭕ 멤버십 적용         |
...
|      항목      |                         8기 오픈미션으로 선정한 편의점                          |                       7기 편의점 4주차 과제                       |
|:------------:|:------------------------------------------------------------------:|:---------------------------------------------------------:|
|    실행 방식     |               웹 UI + JS + REST API + Spring Boot 서버                |                         콘솔 실행 기반                          |
| 언어 & 기술 & 도구 |                   Java + Spring Boot + JPA/MySQL                   |                           Java                            |
|    데이터 관리    |                   DB 기반 영속성 유지 (재고 업데이트 → 서버 반영)                   |       파일(products.md, promotions.md) 입력 기반 / 메모리 관리       |
|   프로모션 처리    |                   사용자 상호작용(모달) → 다중 이슈에 대해 각각 처리                   |                     입력 기반 Y/N 플로우 처리                      |
|    오류 처리     |                 HTTP 예외, 상태 코드(409), JSON 응답 기반 제어                 |     IllegalArgumentException & “[ERROR] 안내 출력” 후 재입력      |
|    결제 플로우    |                       비동기 요청 반복, 트랜잭션 롤백 기반                        |                       명령형 입력 순차 처리                        |
|     멤버십      |                    10% 할인 + 10,000원 한도(현실성 반영)                     |                 남는 수량 30% 할인 + 8,000원 한도                  |
|      제출      |                          GitHub + 웹 실행 결과                          |                     GitHub + 콘솔 실행 결과                     |
|  재고 관리 특이사항  | 일반&프로모션 재고를 구분하지 않음<br>(하나의 상품에 프로모션 적용 시 모든 재고가 프로모션 적용) → 현실성 반영 | 일반&프로모션 재고를 구분함<br>(하나의 상품에 프로모션 적용 시 일반 재고, 프로모션 재고로 나눔) |
